    ...
    async function generateDistribution() {
        ...
        if(!helloFile || !codesFile) return alert("Veuillez sÃ©lectionner les 2 fichiers Excel d'abord.");
        ...
        try {
            ...
            for(let i=1; i<helloData.length; i++) {
                ...
                let tarif = row[12]; // M
                if(email && tarif) {
                    let amountStr = tarif.toString().replace(',', '.').replace('', '').replace('â‚¬', '').trim();
            ...
            if (generatedAssignments.length === 0) {
                alert("Aucun code n'a pu Ãªtre attribuÃ©. VÃ©rifiez le format des fichiers (les colonnes doivent correspondre).");
            }
    ...
    async function saveDistributionToFirebase() {
        if(generatedAssignments.length === 0) return alert("Rien Ã  envoyer.");
        if(!confirm("ÃŠtes-vous sÃ»r de vouloir envoyer ces " + generatedAssignments.length + " codes vers la base de donnÃ©es ? Les utilisateurs concernÃ©s les verront apparaÃ®tre dans leur application.")) return;
        ...
        try {
            ...
            showToast(successCount + " code(s) distribuÃ©(s) avec succÃ¨s !");
